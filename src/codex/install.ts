...
export async function installCodex(options: InstallOptions = {}): Promise<InstallResult> {
  return installFiles('codex', codexFiles(options.homeDir ?? homedir(), options.codexSkillPathMode, options.includeHooks), options);
}

export function codexFiles(homeDir = homedir(), mode: CodexSkillPathMode = 'auto', includeHooks = false): InstallFile[] {
  ...
  if (includeHooks) files.push(...codexHookFiles(homeDir));
  return files;
}
...
function codexHookFiles(homeDir: string): InstallFile[] {
...
function codexHookGuide(scriptPath: string): string {
...
- \`contextbook memory hook-suggest --prompt "앞으로 한국어로 짧게 설명해줘" --source codex --json\` — produce hook-safe suggestion context without applying preferences.
